        ...
        for attempt in range(max_retries):
            try:
                ...
                try:
                    ...
                    if schema_json:
                        # Create minimal schema outline for context
                        schema_context = self.get_minimal_schema_context(schema, page_num)
                    else:
                ...
                if schema == "941-X":
                    if page_num == 0:  # Page 1
                        page_context = """
CRITICAL FIELDS FOR PAGE 1:
- EIN (Employer Identification Number) - usually format XX-XXXXXXX
- Employer Name and Trade Name
- Full Address (street, city, state, ZIP)
- Correction checkboxes (which type of correction)
- Quarter being corrected (1, 2, 3, or 4)
- Year being corrected (e.g., 2020, 2021)"""
                    elif page_num == 1:  # Page 2
                        page_context = """
CRITICAL FIELDS FOR PAGE 2 (Part 2):
Lines 1-27 each have THREE columns:
1. Column 1: Corrected amount (what it should be)
2. Column 2: Amount originally reported
3. Column 3: Difference (Column 1 - Column 2)

Extract ALL three values for each line that has data."""
                    elif page_num == 2:  # Page 3
                        page_context = """
CRITICAL FIELDS FOR PAGE 3 (Part 3):
Lines 28-40 for ERC calculations:
- Line 30: Qualified wages for ERC
- Line 31a: Qualified health plan expenses
- Line 33a, 34, 35a, 36a: Additional qualified wages by category
- Lines with calculations and totals"""
                    elif page_num >= 5:
                        page_context = "\nNOTE: This may be an instructions page. Only extract actual form data if present."
                
                # Enhanced extraction prompt - simplified to avoid token overload
...
5. For 941-X: extract all 3 columns per line

Return a complete JSON with all extracted values. Structure:
{schema_context if len(schema_context) < 5000 else "Extract all form fields following standard IRS " + schema + " structure"}"""
                
                # Use structured output with deterministic settings
                response = client.models.generate_content(
                    model="gemini-2.5-flash-preview-05-20",
                    contents=[extraction_prompt, file_part],
                    config={
                        "max_output_tokens": 20000,  # Increased from 8000
                        "response_mime_type": "application/json",
                        "temperature": 0.0,
                        "top_p": 0.1
                    }
                ...
                try:
                    ...
                    if not page_data or page_data.get("_parse_error"):
                        ...
                        page_data = extract_json_from_llm_response(response_text)
                        if not page_data:
                            # Last resort - try advanced reconstruction
                            page_data = advanced_json_reconstruction(response_text)
                            if not page_data:
                                logger.error(f"All JSON parsing attempts failed for page {page_num + 1}")
                                page_data = {"_extraction_error": "JSON parse failed", "_page": page_num + 1}
    ...
    async def compile_results_with_pro(self, page_results: List[Dict[str, Any]], schema: str) -> Dict[str, Any]:
        """Compile results from multiple pages using Gemini Pro"""
        try:
            ...
            # Log pre-compilation data
            if DEBUG_MODE:
                self._save_debug_compilation("pre_compilation", all_page_data, schema)
            
            # Create compilation prompt
            compilation_prompt = f"""Merge the extracted data from {len(all_page_data)} pages into a single complete JSON object.

MERGING RULES:
1. Combine all page data into one unified structure
2. When the same field appears on multiple pages:
   - Use the non-null value
   - If multiple non-null values exist, use the most complete/recent one
3. Preserve all extracted values - do not discard data
4. Maintain the original structure and field names
5. Remove any page-specific metadata

Page data to merge:
{json.dumps(all_page_data, indent=2)}

Return ONLY the merged JSON object without any wrapper or metadata."""

            # Don't truncate - let Gemini handle large inputs
            # Gemini 2.5 models can handle up to 1M tokens
            if len(compilation_prompt) > 100000:
                logger.warning(f"Large compilation prompt: {len(compilation_prompt)} chars")
                # Only truncate if absolutely massive
                if len(compilation_prompt) > 500000:
                    logger.warning("Extremely large prompt, truncating at 500k chars")
                    compilation_prompt = compilation_prompt[:500000] + "\n\n[TRUNCATED]\n\nMerge all the data shown above."
            
            logger.info(f"Sending compilation request (prompt length: {len(compilation_prompt)} chars)")
            ...
            for attempt in range(3):
                try:
                    response = await asyncio.wait_for(
                        asyncio.to_thread(
                            client.models.generate_content,
                            model="gemini-2.5-pro-preview-06-05",  # Using Pro for compilation as requested
                    ...
                    if response and hasattr(response, 'text') and response.text:
                        ...
                        try:
                            ...
                            if DEBUG_MODE:
                                self._save_debug_compilation("post_compilation", compiled_data, schema)
                            
                            # Clean up any schema artifacts
                            compiled_data = self.clean_extracted_data(compiled_data)
                            
                            await self.update_plan_step("compile", "completed", 
                                f"Compiled {len(all_page_data)} pages")
                    ...
                    logger.error(f"Compilation timeout on attempt {attempt + 1}")
                    if attempt == 2:
                        raise Exception("Compilation timeout after 3 attempts")
                except Exception as e:
                    logger.error(f"Compilation error on attempt {attempt + 1}: {e}")
                    if attempt == 2:
                        raise
            ...
            for page_data in all_page_data:
                if isinstance(page_data.get("data"), dict):
                    merged = self.deep_merge(merged, page_data["data"])
    ...
    def schema_to_example_v2(self, schema: Dict[str, Any], form_type: str) -> Dict[str, Any]:
        """Convert JSON Schema to example structure with contextual examples"""
            ...
            if node_type == "object":
                ...
                for key, value in properties.items():
                    if key not in ["$schema", "title", "description", "required"]:
                        current_path = f"{path}.{key}" if path else key
                        processed = process_schema_node(value, current_path)
                        
                        # Always include the key
                        if isinstance(processed, dict) and processed:
                            result[key] = processed
                        else:
                            # Add contextual example based on field name
                            result[key] = self._get_field_example(key, current_path, form_type)
                return result
                ...
                # Return example based on field context
                return self._get_field_example(path.split('.')[-1], path, form_type)
    ...
    def _get_field_example(self, field_name: str, path: str, form_type: str) -> Any:
        """Get contextual example for a field"""
        field_lower = field_name.lower()
        
        # Common patterns
        if "ein" in field_lower:
            return "XX-XXXXXXX"
        elif "name" in field_lower:
            if "business" in field_lower or "employer" in path.lower():
                return "Company Name LLC"
            else:
                return "John Doe"
            ...
            return "MM/DD/YYYY"
        elif "quarter" in field_lower:
            return 1
        elif "year" in field_lower:
            return 2021
        elif any(x in field_lower for x in ["checkbox", "checked", "box"]):
            return False
        
        # Form-specific examples
        if form_type == "941-X" and "line" in path.lower():
            if "corrected" in field_lower:
                return 0.00
            elif "original" in field_lower:
                return 0.00
            elif "difference" in field_lower:
                return 0.00
    ...
    def get_minimal_schema_context(self, schema: str, page_num: int) -> str:
        """Get minimal schema context for a specific page"""
        if schema == "941-X":
            if page_num == 0:  # Page 1
                return """{
  "formMetadata": {...},
  "employerInfo": {"ein": "", "name": "", "address": {...}},
  "correctionInfo": {"quarter": 0, "year": 0},
  "correctionType": {"isAdjustedReturn": false, ...}
}"""
            elif page_num == 1:  # Page 2
                return """{
  "lines": {
    ...
    ... (lines 1-27 with same structure)
  }
}"""
            elif page_num == 2:  # Page 3
                return """{
  ...
  "lines": {
    ...
    ... (ERC related lines)
                ...
                return "Extract all visible form data"
        else:
            return "Extract all visible form data following standard structure"
