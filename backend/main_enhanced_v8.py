...
Return ONLY the JSON with extracted values."""
    ...
    async def compile_results_with_pro(self, page_results: List[Dict[str, Any]], schema: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Create a simple merge prompt
            compilation_prompt = f"""Merge the extracted data from {len(all_page_data)} pages into a single JSON object.

Page data to merge:
{json.dumps(all_page_data, indent=2)}

Instructions:
1. Combine all page data into one complete JSON
2. When same field appears on multiple pages, use non-null value
3. Return ONLY the merged JSON object
4. Do not include page numbers or metadata in final result"""
