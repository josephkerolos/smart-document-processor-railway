    ...
    def extract_single_page(self, page_num: int, page_content: bytes, schema: str, total_pages: int) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                IMPORTANT CONTEXT:
                - This is ONLY page {page_num + 1} out of {total_pages} total pages
                - Extract ALL data visible on this page
                - The data will be merged with other pages to complete the full form
                - Some fields may be empty on this page but filled on others
                - Include partial data even if incomplete
                
                TARGET STRUCTURE (fill with actual values from the document):
                {schema_context}
                
                EXTRACTION RULES:
                1. Extract ACTUAL VALUES from the document, not schema definitions
                2. Replace null values with the actual data you see on the page
                3. For fields not visible on this page, keep as null
                4. Extract numbers as numbers (not strings), e.g., 12345.67 not "12345.67"
                5. Extract text exactly as shown (preserve case, spacing)
                6. For checkboxes, use true/false based on whether they're marked
                7. For 941-X: Pay special attention to three-column format (corrected, original, difference)
                8. CRITICAL: Do NOT return schema definitions with "type", "properties", etc.
                9. Return the actual extracted data values from the PDF
                
                ORGANIZE BY FORM STRUCTURE:
                - Parts → Lines → Fields (a, b, c)
                - Keep hierarchical organization
                - Match the form's layout
                
                CRITICAL JSON FORMATTING:
                - Your response must be valid JSON that starts with {{ and ends with }}
                - Return actual extracted values, not schema definitions
                - Example of CORRECT response: {{"employerInfo": {{"ein": "12-3456789", "name": "ABC Company"}}}}
                - Example of WRONG response: {{"employerInfo": {{"type": "object", "properties": {{...}}}}}}
                - Use null for missing/not visible values
                
                Return ONLY the JSON object with actual extracted data. No explanations, no markdown, no code blocks.
                """
...
Return ONLY the JSON with extracted values. If a field is not visible on this page, use null."""
    ...
    async def compile_results_with_pro(self, page_results: List[Dict[str, Any]], schema: str) -> Dict[str, Any]:
        ...
        try:
            ...
            You are compiling extracted data from {len(page_results)} pages of a {schema} form into a single, complete JSON structure.
            {failed_pages_note}{partial_pages_note}
            
            EXTRACTED DATA FROM PAGES (these contain actual values from the document):
            {json.dumps(all_page_data, indent=2)}
            
            TARGET STRUCTURE (merge all page data into this format):
            {schema_context}
            
            COMPILATION RULES:
            1. Merge all extracted VALUES from different pages into one complete JSON
            2. This is NOT about schema definitions - merge the actual extracted data
            3. When the same field appears on multiple pages, prefer non-null values
            4. Combine data logically - if page 1 has employerInfo and page 2 has lines, merge both
            5. Preserve all actual values extracted from the document
            6. For 941-X forms: Ensure three-column data (corrected, original, difference) is preserved
            7. Calculate any missing "difference" values if you have corrected and original
            8. Keep all line numbers and sub-fields (a, b, c) organized properly
            
            CRITICAL REQUIREMENTS:
            - Return actual extracted data values, NOT schema definitions
            - Do NOT include "type", "properties", "description" or other schema keywords
            - Example of CORRECT output: {{"employerInfo": {{"ein": "12-3456789", "name": "ABC Corp"}}}}
            - Example of WRONG output: {{"employerInfo": {{"type": "object", "properties": {{...}}}}}}
            
            ORGANIZATION:
            - Follow the hierarchical structure shown in the target format
            - Group by Parts → Lines → Fields as shown in the extracted data
            - Maintain clean JSON formatting
            
            Return ONLY the compiled JSON with all actual extracted data merged together. No explanations.
            """
            ...
            if response_text:
                ...
                if response_text.strip() == "":
                    logger.error("Response text contains only whitespace!")
            ...
            # Log first 500 chars of response for quick debugging
    ...
    def schema_to_example(self, schema: Dict[str, Any]) -> Dict[str, Any]:
        """Convert JSON Schema to example structure with placeholder values"""
        def process_schema_node(node):
            ...
            if node_type == "object":
                ...
                item_example = process_schema_node(items)
                return [] if item_example is None else [item_example]
                
            elif node_type == "string":
                # Return null for string fields (to be filled by extraction)
                return None
                ...
                # Return null for boolean fields
                return None
