    ...
    def extract_single_page(self, page_num: int, page_content: bytes, schema: str, total_pages: int) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if schema_json:
                    # Convert schema to example format with minimal nulls
                    schema_example = self.schema_to_example_v2(schema_json, schema)
...
- Lines with calculations and totals"""
                ...
                extraction_prompt = f"""You are extracting data from page {page_num + 1} of {total_pages} of a {schema} form.

CRITICAL INSTRUCTIONS:
1. Look at EVERY field visible on the page
2. Extract ALL values you can see - numbers, text, checked boxes
3. Do NOT leave fields as null if you can see a value
4. For checkboxes: true if marked/checked, false if empty box visible, null if not on page
5. For numbers: extract exact values including decimals
6. For text: extract complete text as shown
{page_context}

TARGET STRUCTURE TO FILL:
{schema_context}

EXTRACTION RULES:
- If you see a value for a field, YOU MUST extract it
- Only use null for fields that are NOT visible on this page
- For 941-X lines: extract all 3 columns (corrected, original, difference)
- Include negative numbers with minus sign if shown
- Preserve exact formatting of EINs (XX-XXXXXXX)

Return ONLY the JSON with extracted values. Every visible value must be captured."""
...
Return ONLY the merged JSON object without any wrapper or metadata."""
            ...
            # Truncate if needed
            if len(compilation_prompt) > 30000:
                logger.warning("Compilation prompt too long, truncating...")
                compilation_prompt = compilation_prompt[:30000] + "\n\n[TRUNCATED]\n\nMerge all the data shown above."
